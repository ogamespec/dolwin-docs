# GameCube — Disk Interface (DI)

> **DI** (Disk Interface) is the block inside the **Flipper** ASIC that connects
> the console to the GameCube disk drive unit (DDU). The drive is an intelligent,
> command-driven device with its own microcontroller and firmware, so everything
> the console "knows" about the disk is sent as a **command packet** over a simple
...
Data flow for a **read from disk** (the common case):

```
drive (DIDD) ──io_didctl──▶ io_dififo ── (32-byte full line) ──io_dimctl──▶ main memory
```

For a **write to disk** the same FIFO is filled from memory by `io_dimctl` and
...
| `DICOVER` | 1 | I | Disk cover switch. `1` = cover open, `0` = cover closed |
...
Streamed disk audio is carried on separate pins into Flipper and crosses into the
...
Software drives the disk controller entirely through the register file. A
...
- **DMA mode** (`DMA`=1): data is streamed to/from **main memory** in 32-byte
  ...
  the normal bulk data path (game data reads, disk-ID reads, inquiries).
...
| `0xA8` | Read | DMA | Read disk data. `subcmd` (`CMDBUF3`), `CMDBUF1 = offset >> 2`, `CMDBUF2` = byte length, `MAR`/`LEN` = destination, `CR = DMA \| TSTART` |
| `0xA8` sub `0x40` | Read disk ID | DMA | Read the 32-byte disk header (`DVDDiskID`) from the start of the disk; otherwise the same as `0xA8` |
...
usual value is `0x00` for a normal data read and `0x40` for the disk-ID read.
...
PI general/reset register. `CONFIG[2]` (`DIRSTB`) at `0x0C003024` is the disk-interface
...
| `DICVR[CVRINT]` (2) | `DICVR[CVRINTMSK]` (1) | Disk cover closed |
...
- `specs/architecture/processor-interface.md` — §3 register access (16-bit
  ...
  disk-interface reset, and the PI error model for the IO register space.
...
- `RE/DVD/dvdfs.txt` — disk filesystem / command-block notes.
